 ...
 */
export async function getArtworkRecommendations(limit = 10): Promise<Artwork[]> {
  const response = await fetch(`${API_URL}/api/artworks/recommendations?limit=${limit}`)
...
export async function getTrendingArtworks(limit = 6): Promise<Artwork[]> {
  const response = await fetch(`${API_URL}/api/artworks/trending?limit=${limit}`)
 ...
 */
export async function toggleSaveArtwork(artworkId: string, userId: string): Promise<{ saved: boolean }> {
...
export async function getSavedArtworks(userId: string): Promise<Artwork[]> {
  const response = await fetch(`${API_URL}/api/artworks/saved?userId=${userId}`)
