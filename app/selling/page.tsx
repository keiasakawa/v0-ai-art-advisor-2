...
export default function SellingDashboard() {
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      ...
      const fetchArtworks = async () => {
        ...
        setListings(
          artworks.filter((a) => a.status === "listed" || a.status === "sold"),
        );
  ...
  const handleListArtwork = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        const artwork = collectionArtworks.find(
          (a) => a.id === selectedArtwork,
        );
        if (artwork) {
          setListings([
            { ...artwork, status: "listed", desiredPrice: price },
            ...listings,
          ]);
          setCollectionArtworks(
            collectionArtworks.filter((a) => a.id !== selectedArtwork),
          );
        }
        // Reset form
  ...
  const openEditDialog = (listing: Artwork) => {
    setEditingListingId(listing.id);
    setSelectedArtwork(listing.id);

    // prefill values
    setListingPrice(listing.desiredPrice || "");
    setAuctionStartingBid(listing.desiredPrice || "");

    // optional if you store type
    setListingType("fixed");
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          <Dialog open={isListDialogOpen} onOpenChange={setIsListDialogOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>
                  {editingListingId ? "Edit Listing" : "List Artwork for Sale"}
                </DialogTitle>

                <DialogDescription>
                  {editingListingId
                    ? "Update your listing details"
                    : "Select an artwork from your collection to list"}
                </DialogDescription>
              </DialogHeader>
                ...
                <>
                  <div className="space-y-3">
                    ...
                    <RadioGroup
                      value={listingType}
                      onValueChange={(value) =>
                        setListingType(value as "fixed" | "auction")
                      }
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setAuctionStartingBid(e.target.value)
                            }
                            className="pl-9"
                          />
                        </div>
